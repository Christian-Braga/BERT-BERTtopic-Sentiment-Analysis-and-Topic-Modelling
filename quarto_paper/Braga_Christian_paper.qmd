...
While for the inference datasets i followed the same process but without shuffuled the data and without aggregating the comments of the different videos in one dataset. Before applying the models, the comments underwent a series of preprocessing steps to prepare them for analysis. This preprocessing involved converting all text to lowercase for uniformity and removing any URLs to clean the data. Additionally, unnecessary spaces were reduced to a single space, and mentions and hashtags, indicated by '@' or '#', were stripped from the text. To facilitate effective model processing, the text was tokenized using the BERT model's tokenizer (bert-base-uncased), which breaks down the text into tokens suitable for input into the model.
...
To conclude this temporal analysis by specific news network, we can affirm that the evolution of the conflict has led to greater polarization in the opinions expressed by YouTube users. CNN emerges as the channel where this trend is most pronounced, being the only one where comments supporting a faction utnumbered neutral ones in 2024 despite having the highest incidence of the latter in 2023. Sky News has remained the most balanced channel over time, and Al Jazeera has seen the greatest percentage increase in pro-Palestinian comments from 2023 to 2024 and the largest decrease in pro-Israel comments among the three, yet still retaining a majority.
